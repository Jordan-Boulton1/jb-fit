...
class CustomSignupFormTests(TestCase):
    ...
    def test_form_invalid_first_name(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertIn('first_name', form.errors)
        self.assertEqual(form.errors['first_name'], ['First name can only contain alphabetical characters.'])
    ...
    def test_form_invalid_last_name(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertIn('last_name', form.errors)
        self.assertEqual(form.errors['last_name'], ['Last name can only contain alphabetical characters.'])

    def test_form_date_of_birth_in_future(self):
        invalid_data = self.valid_data.copy()
        invalid_data['date_of_birth'] = date.today() + timedelta(days=1)  # Future date
        form = CustomSignupForm(data=invalid_data)
        self.assertFalse(form.is_valid())
        self.assertIn('date_of_birth', form.errors)
        self.assertEqual(form.errors['date_of_birth'], ['Date of birth cannot be in the future.'])
...
class CustomLoginFormTest(TestCase):
    ...
    def test_form_initialization(self):
        ...
        self.assertEqual(form.fields['login'].widget.attrs['class'], 'form-control')
        self.assertEqual(form.fields['login'].widget.attrs['placeholder'], 'Email address')
        ...
        self.assertEqual(form.fields['remember'].widget.attrs['class'], 'form-check-input')
    ...
    def test_form_invalid_email(self):
        ...
        self.assertEqual(form.errors['login'], ['Enter a valid email address.'])
...
class UserProfileFormTest(TestCase):
    ...
    def test_form_missing_required_fields(self):
        ...
        required_fields = ['email', 'phone_number', 'address', 'date_of_birth', 'current_weight', 'height', 'goal_weight']
    ...
    def test_duplicate_email(self):
        """Test that form validation fails if the email already exists for another user."""
    ...
    def test_invalid_phone_number(self):
        ...
        self.assertEqual(form.errors['phone_number'], ['Phone number must contain only digits.'])
    ...
    def test_invalid_current_weight(self):
        ...
        self.assertEqual(form.errors['current_weight'], ['Current weight must be greater than zero.'])
    ...
    def test_invalid_height(self):
        ...
        self.assertEqual(form.errors['height'], ['Height must be greater than zero.'])
    ...
    def test_invalid_goal_weight(self):
        ...
        self.assertEqual(form.errors['goal_weight'], ['Goal weight must be greater than zero.'])

    def test_future_date_of_birth(self):
        """Test that form validation fails if date of birth is in the future."""
        invalid_data = self.valid_data.copy()
        invalid_data['date_of_birth'] = (datetime.today().date() + timedelta(days=1)).isoformat()
        form = UserProfileForm(data=invalid_data, instance=self.profile)
        self.assertFalse(form.is_valid())
        self.assertIn('date_of_birth', form.errors)
        self.assertEqual(form.errors['date_of_birth'], ['Date of birth cannot be in the future.'])
...
class WeightLogFormTest(TestCase):
    ...
    def test_form_initialization(self):
        ...
        form = WeightLogForm()
        self.assertEqual(form.fields['weight'].widget.attrs.get('class'), 'form-control')
        self.assertEqual(form.fields['weight'].widget.attrs.get('step'), '0.1')

    def test_form_valid_data(self):
        """Test that the form is valid with correct data."""
        form = WeightLogForm(data=self.valid_data)
        self.assertTrue(form.is_valid(), f"Expected form to be valid with data: {self.valid_data}")
    ...
    def test_clean_weight_positive(self):
        ...
        form = WeightLogForm(data=self.valid_data)
        self.assertTrue(form.is_valid(), "Form should be valid with positive weight")
    ...
    def test_clean_weight_negative(self):
        ...
        self.assertFalse(form.is_valid())
        self.assertIn('weight', form.errors)
        self.assertEqual(form.errors['weight'], ['Value must be greater than zero.'])
...
class ProgressPictureFormTest(TestCase):
    ...
    def test_form_valid_data(self):
        """Test that the form is valid with correct data."""
        form = ProgressPictureForm(data={}, files=self.valid_data)  # Files are passed separately
        self.assertTrue(form.is_valid(), f"Expected form to be valid with data: {self.valid_data}")
    ...
    def test_form_invalid_file_type(self):
        ...
        form = ProgressPictureForm(data={}, files=invalid_data)  # Files are passed separately
        self.assertFalse(form.is_valid())
        self.assertIn('progress_image', form.errors)
        # Check if there's an error related to file type (depends on validation settings)
        self.assertTrue(any("image" in error for error in form.errors['progress_image']), 
                        "Expected error about file type but got: {}".format(form.errors['progress_image']))
