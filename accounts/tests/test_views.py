...
class AccountsViewsTest(TestCase):
    ...
    def test_add_weight_log(self):
        ...
        self.assertRedirects(response, reverse('profile'))
        self.assertTrue(
            WeightLog.objects.filter(user=self.user, weight=70).exists()
        )
    ...
    def test_upload_progress_picture(self):
        with open('path_to_image.jpg', 'rb') as img:
            response = self.client.post(
                reverse('upload_progress_picture'), {
                    'image': SimpleUploadedFile(
                        img.name, img.read(), content_type='image/jpeg'
                    )
                }
            )
        self.assertRedirects(response, reverse('profile'))
        self.assertTrue(
            ProgressPicture.objects.filter(user=self.profile).exists()
        )
